...
export const registerUser = createAsyncThunk('user/register', async (user: user & { password: string }) => {
    var data = await fetch('https://ecommerce.back.zykov.xyz/api/auth/register', {
        method: 'POST',
        headers: new Headers({
...
export const authUser = createAsyncThunk('user/auth', async (schema: {
    ...
    var data = await fetch('https://ecommerce.back.zykov.xyz/api/auth/authenticate', {
        method: 'POST',
        headers: new Headers({
...
export const getMe = createAsyncThunk('user/get', async () => {
    var data = await fetch('https://ecommerce.back.zykov.xyz/api/user/get/0', {
        method: 'GET',
        headers: new Headers({
            'Authorization': 'Bearer ' + getToken(),
            'Content-Type': 'application/json'
...
export const updateUser = createAsyncThunk('user/update', async (user: user) => {
    var data = await fetch('https://ecommerce.back.zykov.xyz/api/user/update/0', {
        method: 'PUT',
        headers: new Headers({
            'Authorization': 'Bearer ' + getToken(),
            'Content-Type': 'application/json'
