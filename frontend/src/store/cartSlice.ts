...
export const getCart = createAsyncThunk('cart/get', async () => {
    ...
    var data = await fetch('https://ecommerce.back.zykov.xyz/api/cart/get/0', {
        method: 'GET',
        headers: new Headers({
...
export const updateCart = createAsyncThunk('cart/update', async (cart: pair[]) => {
    ...
    var data = await fetch('https://ecommerce.back.zykov.xyz/api/cart/add', {
        method: 'PUT',
        headers: new Headers({
            'Authorization': 'Bearer ' + getToken(),
            'Content-Type': 'application/json'
...
export const addItemToCart = createAsyncThunk('cart/update', async (id: number) => {
    ...
    var data = await fetch('https://ecommerce.back.zykov.xyz/api/cart/add/' + id, {
        method: 'PUT',
        headers: new Headers({
            'Authorization': 'Bearer ' + getToken(),
            'Content-Type': 'application/json'
